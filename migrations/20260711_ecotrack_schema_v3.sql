...
CREATE INDEX IF NOT EXISTS idx_collect_points_categorie ON public.collect_points(categorie);
CREATE INDEX IF NOT EXISTS idx_waste_scan_user_id ON public.waste_scan_results(user_id);
CREATE INDEX IF NOT EXISTS idx_itineraire_collecteur_id ON public.itineraire_arrets(collecteur_id);
CREATE INDEX IF NOT EXISTS idx_echanges_points_user_id ON public.echanges_points(user_id);
...
-- ---------- collect_points ----------
CREATE POLICY IF NOT EXISTS "collect_points_select_public" ON public.collect_points
  FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "collect_points_write_admin" ON public.collect_points
  FOR INSERT TO authenticated WITH CHECK (public.is_admin() OR public.has_role('mairie'));
CREATE POLICY IF NOT EXISTS "collect_points_update_admin" ON public.collect_points
  FOR UPDATE USING (public.is_admin() OR public.has_role('mairie'));
CREATE POLICY IF NOT EXISTS "collect_points_delete_admin" ON public.collect_points
  FOR DELETE USING (public.is_admin() OR public.has_role('mairie'));

-- ---------- signalements ----------
CREATE POLICY IF NOT EXISTS "signalements_select_owner_or_privileged" ON public.signalements
  FOR SELECT USING (
    user_id = auth.uid() OR public.is_admin() OR public.has_role('mairie') OR public.has_role('collecteur')
  );
CREATE POLICY IF NOT EXISTS "signalements_insert_auth" ON public.signalements
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY IF NOT EXISTS "signalements_update_owner_or_privileged" ON public.signalements
  FOR UPDATE USING (
    user_id = auth.uid() OR public.is_admin() OR public.has_role('mairie')
  );
CREATE POLICY IF NOT EXISTS "signalements_delete_owner_or_admin" ON public.signalements
  FOR DELETE USING (user_id = auth.uid() OR public.is_admin());

-- ---------- signalement_etapes ----------
CREATE POLICY IF NOT EXISTS "etapes_select_related" ON public.signalement_etapes
  FOR SELECT USING (
    public.is_admin()
    OR signalement_id IN (SELECT id FROM public.signalements WHERE user_id = auth.uid())
  );
CREATE POLICY IF NOT EXISTS "etapes_insert_privileged" ON public.signalement_etapes
  FOR INSERT TO authenticated WITH CHECK (public.is_admin() OR public.has_role('mairie'));

-- ---------- waste_scan_results ----------
CREATE POLICY IF NOT EXISTS "scan_select_own" ON public.waste_scan_results
  FOR SELECT USING (user_id = auth.uid());
CREATE POLICY IF NOT EXISTS "scan_insert_own" ON public.waste_scan_results
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());

-- ---------- collecteur_stats ----------
CREATE POLICY IF NOT EXISTS "collecteur_stats_select_own_or_admin" ON public.collecteur_stats
  FOR SELECT USING (user_id = auth.uid() OR public.is_admin());
CREATE POLICY IF NOT EXISTS "collecteur_stats_update_own" ON public.collecteur_stats
  FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY IF NOT EXISTS "collecteur_stats_insert_own" ON public.collecteur_stats
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());

-- ---------- itineraire_arrets ----------
CREATE POLICY IF NOT EXISTS "itineraire_select_own_or_admin" ON public.itineraire_arrets
  FOR SELECT USING (collecteur_id = auth.uid() OR public.is_admin());
CREATE POLICY IF NOT EXISTS "itineraire_write_admin" ON public.itineraire_arrets
  FOR INSERT TO authenticated WITH CHECK (public.is_admin());

-- ---------- recompenses ----------
CREATE POLICY IF NOT EXISTS "recompenses_select_public" ON public.recompenses
  FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "recompenses_write_admin" ON public.recompenses
  FOR INSERT TO authenticated WITH CHECK (public.is_admin());
CREATE POLICY IF NOT EXISTS "recompenses_update_admin" ON public.recompenses
  FOR UPDATE USING (public.is_admin());

-- ---------- echanges_points ----------
CREATE POLICY IF NOT EXISTS "echanges_select_owner_or_admin" ON public.echanges_points
  FOR SELECT USING (user_id = auth.uid() OR public.is_admin());
CREATE POLICY IF NOT EXISTS "echanges_insert_owner" ON public.echanges_points
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY IF NOT EXISTS "echanges_update_admin" ON public.echanges_points
  FOR UPDATE USING (public.is_admin());

-- ---------- produits_marketplace ----------
CREATE POLICY IF NOT EXISTS "produits_select_public" ON public.produits_marketplace
  FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "produits_write_filiere_admin" ON public.produits_marketplace
  FOR INSERT TO authenticated WITH CHECK (public.is_admin() OR public.has_role('filiere'));
CREATE POLICY IF NOT EXISTS "produits_update_filiere_admin" ON public.produits_marketplace
  FOR UPDATE USING (public.is_admin() OR public.has_role('filiere'));

-- ---------- commandes / lignes_commande ----------
CREATE POLICY IF NOT EXISTS "commandes_select_owner_or_admin" ON public.commandes
  FOR SELECT USING (user_id = auth.uid() OR public.is_admin());
CREATE POLICY IF NOT EXISTS "commandes_insert_owner" ON public.commandes
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY IF NOT EXISTS "commandes_update_owner_or_admin" ON public.commandes
  FOR UPDATE USING (user_id = auth.uid() OR public.is_admin());

CREATE POLICY IF NOT EXISTS "lignes_select_related" ON public.lignes_commande
  FOR SELECT USING (
    public.is_admin()
    OR commande_id IN (SELECT id FROM public.commandes WHERE user_id = auth.uid())
  );
CREATE POLICY IF NOT EXISTS "lignes_insert_related" ON public.lignes_commande
  FOR INSERT TO authenticated WITH CHECK (
    commande_id IN (SELECT id FROM public.commandes WHERE user_id = auth.uid())
  );

-- ---------- impact_carbone ----------
CREATE POLICY IF NOT EXISTS "impact_select_own_or_admin" ON public.impact_carbone
  FOR SELECT USING (user_id = auth.uid() OR public.is_admin());
CREATE POLICY IF NOT EXISTS "impact_upsert_admin" ON public.impact_carbone
  FOR INSERT TO authenticated WITH CHECK (public.is_admin());
CREATE POLICY IF NOT EXISTS "impact_update_admin" ON public.impact_carbone
  FOR UPDATE USING (public.is_admin());

-- ---------- dashboard_mairie ----------
CREATE POLICY IF NOT EXISTS "dashboard_select_mairie_admin" ON public.dashboard_mairie
  FOR SELECT USING (public.has_role('mairie') OR public.is_admin());
CREATE POLICY IF NOT EXISTS "dashboard_write_admin" ON public.dashboard_mairie
  FOR INSERT TO authenticated WITH CHECK (public.is_admin());
CREATE POLICY IF NOT EXISTS "dashboard_update_admin" ON public.dashboard_mairie
  FOR UPDATE USING (public.is_admin());
...
CREATE POLICY IF NOT EXISTS "votes_select_public" ON public.votes_communautaires
  FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "votes_insert_own" ON public.votes_communautaires
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
...
CREATE INDEX IF NOT EXISTS idx_auth_email_otps_email ON public.auth_email_otps (email);
CREATE INDEX IF NOT EXISTS idx_auth_email_otps_user_id ON public.auth_email_otps (user_id);
CREATE INDEX IF NOT EXISTS idx_auth_email_otps_sent_at ON public.auth_email_otps (sent_at);
